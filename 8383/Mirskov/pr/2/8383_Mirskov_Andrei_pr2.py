...
def drawResults(data, label, prediction):
    ...
    plt.grid()
    plt.show()
(train_data, train_label), (test_data, test_label) = genData(20000)
...
y_val = train_label[:len(train_label)//2]
partial_y_train = train_label[len(train_label)//2:]

H = model.fit(partial_x_train,partial_y_train,epochs=50,batch_size=512,validation_data=(x_val, y_val))
