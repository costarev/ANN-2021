...
partial_x_train = train_data[len(train_data) * 2 // 10:]  # данные для обучения
...
# добавление слоев
model.add(layers.Dense(32, activation='relu', input_shape=(2,)))  # input_shape - x and y
model.add(layers.Dense(32, activation='relu'))
model.add(layers.Dense(1, activation='sigmoid'))

model.compile(optimizer='rmsprop', loss='binary_crossentropy', metrics=['accuracy'])

H = model.fit(partial_x_train, partial_y_train, epochs=100, batch_size=50, validation_data=(x_val, y_val))
# batch_size - пропускаем 50 элементов и меняем веса
